...
func _ready() -> void:
	Global.temp_remove_AOT.connect(removeAOT)
	for i in get_parent().get_child_count():
		var node = get_parent().get_child(i)
		if node.name == "loopeditor":
			always_on_top = false
			Global.temp_remove_AOT.emit()
...
func _on_events_node_item_selected(index: int) -> void:
	...
	if index == 5: # custom, shows an dialog and minimizes this window
		always_on_top = false
	...
	if index not in [4, 8, 9, 5]: # If the selected one isn't a special event, add it
...
func receive_value(value , _empty = null):
	...
	if events_list.get_selected_id() == 5: # For valuechange
			mode = MODE_WINDOWED
